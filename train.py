...
def prepare_dataset(config: dict, stage: str) -> Path:
    ...
    dataset_name = config.get("dataset", {}).get("name", "tobil/qmd-query-expansion-train-v2")
    ...
    eval_ratio = config.get("dataset", {}).get("eval_split", 0.1)
...
def train_sft(config: dict):
    ...
    adapter_path = Path("adapters") / "sft"
